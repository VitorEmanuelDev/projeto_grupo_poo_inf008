...
public class Genius extends JPanel implements ActionListener, MouseListener {
	...
	private void criarEntradaDeDadosInicial() {
		...
		Integer tamanhoLista = (Integer) JOptionPane.showInputDialog(null, "Escolha o n√∫mero de jogadores:", 
		...
		String [] velocidades = {"normal", "lento", "r√°pido"};
	...
	private void paint(Graphics2D g) {
		...
		if (jogoTerminado || jogadorErrou) {
			...
			// se indexJogadorAtual for 0 ent√£o significa que o ultimo jogador errou, nesse caso
	...
	/**
	 * Listener do bot√£o de inserir dados iniciais
	 */
	...
	private class botaoSalvarCarregarListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (botaoSalvarCarregar.getText() == "CARREGAR CAMPEONATO") {
				...
				// somente avan√ßamos se foi selecionado um arquivo v√°lido
				if (opcaoSelecionada != JFileChooser.APPROVE_OPTION) return;

				// conter√° dados necess√°rios para continuar o campeonato (indexJogadorAtual, moduloVelocidade, offsetFase)
				...
				try {
					...
					// le primeira linha (ou seja at√© que cheguemos no \n)
					...
					excecao.printStackTrace();
					JOptionPane.showConfirmDialog(null, 
							"Falha! Arquivo inv√°lido", "Erro ao Carregar Campeonato", JOptionPane.DEFAULT_OPTION);
					return;
				}
				...
				jogoRodando = true;
				alertaJogoTerminado(false);
				toques = 0; // zera contador de atualiza√ß√µes da tela
				iniciarUmaSequencia();
			} else {
				...
				try {
					...
					// caso o usu√°rio n√£o adicionar .genius no final do nome do arquivo, vamos adicion√°-lo
	...
	private class botaoAjudaListener implements ActionListener {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			JOptionPane.showMessageDialog(null,"O GENIUS È um jogo de memÛria visual e sonora, com trÍs nÌveis de velocidade\n"
					+ "(normal, lento e r·pido), e de dificuldade (f·cil, mÈdio, difÌcil).\n\n" 
					+ "Cada cor tem um som correspondente, o jogo vai gerar sequÍncias que o\n"
					+ "jogador deve seguir. VocÍ precisa clicar em cima dela (repetir a sequÍncia).\n\n"
					+ "A cada jogada, o GENIUS acende uma luz e emite um som a mais, formando\n"
					+ "uma sequÍncia diferente cada vez maior, deve ser repetida pelo jogador.\n\n"
					+ "Se vocÍ errar a repetiÁ„o, a sua rodada termina e vai para o prÛximo jogador,\n"
					+ "se n„o houver mais jogadores, o jogo termina.\n\n"
					+ "O jogo pode ser pausado e retomado posteriormente do mesmo ponto e caso,\n"
					+ "queira pode salvar o estado atual do campeonato em um arquivo e carreg·-lo.\n\n"
					
					+ "Trabalho Avaliativo da disciplina ProgramaÁ„o Orientada a Objetos.\n\n"
					+ "Discentes: George Neres, Jean Andrade, Lucas FonsÍca e Vitor Emanuel"
			);			
			}
		}           
	// Mouse Listeners:
